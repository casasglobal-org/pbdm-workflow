...
while (my $file = readdir(DIR))
{
	if ($file =~ /.\.tab$/)
	{
		...
		print GNUPLOT "set size 1.5,0.5\n";
		print GNUPLOT "set out \"$file-PLOTa.png\"\n";
		print GNUPLOT "set autoscale\n";
		print GNUPLOT "set xrange [ 0.5:20.5 ]\n";
		print GNUPLOT "set tics out\n";
		print GNUPLOT "set xtics nomirror 1,1,20\n";
		print GNUPLOT "set mxtics 2\n";
		print GNUPLOT "set ytics auto\n";
		print GNUPLOT "set title \"Extent of raster categories by administrative regions\"\n";
		print GNUPLOT "set ylabel \"Area (Km^{2})\" 0,-4\n";
		print GNUPLOT "set xlabel \"Administrative regions\"\n";
