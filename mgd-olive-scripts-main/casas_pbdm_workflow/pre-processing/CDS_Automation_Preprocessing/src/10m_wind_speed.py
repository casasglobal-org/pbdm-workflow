...
def download_and_extract(client, variable, year, month_str, days, label, s3_bucket):
    ...
    try:
        ...
        if variable == '10m_wind_speed':
            process_10m_wind_speed(client, common_params, year, month_str, label, s3_bucket)
            
        logger.info(f"Successfully processed and extracted {variable}-{statistics} for {month_str}-{year}")
...
def process_10m_wind_speed(client, common_params, year, month_str, label, s3_bucket):
...
def upload_to_s3(output_file_path, s3_bucket, year, label, statistic):
    ...
    if statistic == '24_hour_mean':
        s3_key = f"AgERA5/raw/{year}/{label}-10m-Mean/Monthly/{file_name}"
