...
def download_and_extract(client, variable, year, month_str, days, label, s3_bucket):
    ...
    try:
        ...
        if variable == 'solar_radiation_flux':
            process_solar_radiation_flux(client, common_params, year, month_str, label, s3_bucket)
...
def process_solar_radiation_flux(client, common_params, year, month_str, label, s3_bucket):
